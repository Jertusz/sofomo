...
class IPDetail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        response = requests.get(f"{API_URL}{ip_from_request}?access_key={settings.REMOTE_API_KEY}")
...
class URLDetail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if IPAddress.objects.filter(url=wanted_url).exists():
            ...
            serialized_ip = DetailedIPSerializer(ip_in_db)
            return Response({"message": "This url already exists in database", "data": serialized_ip.data}, status=status.HTTP_302_FOUND)
